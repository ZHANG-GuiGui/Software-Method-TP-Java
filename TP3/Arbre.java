class Arbre <T>{
	...
	public void deriveRatio(){
		if (this.valeur().equals("/")){
			...
			this.affecterFilsDroit(new Arbre("^",denom,new Arbre("2")));
		}
	}
	// This function follows the standard derivation protocol which suits for every type of equations
	// except for
	// Attention: ^ should be SA^number;
	public Arbre<T> deriver(){
		String v = this.valeur().toString();
		if (v.matches("^[0-9]+$")){
			return new Arbre("0");
		}else if (v.equals("x")){
			return new Arbre("1");
		}else if (v.equals("+")||v.equals("-")){
			return new Arbre(v,this.filsGauche().deriver(),this.filsDroit().deriver());
		}else if (v.equals("*")){
			Arbre term1 = new Arbre("*", this.filsGauche().deriver(),this.filsDroit());
			Arbre term2 = new Arbre("*", this.filsGauche(), this.filsDroit().deriver());
			return new Arbre("+",term1,term2);
		}else if (v.equals("/")){
			Arbre deno = new Arbre("^", this.filsDroit(), new Arbre("2"));
			Arbre nume1 = new Arbre("*", this.filsGauche().deriver(), this.filsDroit());
			Arbre nume2 = new Arbre("*", this.filsGauche(), this.filsDroit().deriver());
			Arbre nume = new Arbre("-",nume1,nume2);
			return new Arbre("/",nume,deno);
		}else if (v.equals("^")){
			Arbre term1 = new Arbre("*",this.filsDroit(),new Arbre("^",this.filsGauche(),new Arbre("-",this.filsDroit(),new Arbre("1"))));
			return new Arbre("*",term1,this.filsGauche().deriver());
		}
		return null;
	}
	public boolean petitArbre(Arbre<T> arb){
		if (arb.hauteur()==2){
			return true;
		}
		return false;
	}
	public void simplifiDerive(Double valeur){
		this.simplification();
		if (this.hauteur()!=1){
			if (!this.petitArbre(this)){
				this.filsGauche().simplifiDerive(valeur);
				this.filsDroit().simplifiDerive(valeur);
			}else{
				if (this.filsGauche().valeur().toString().matches("^[0-9]+$")&&this.filsDroit().valeur().toString().matches("^[0-9]+$")){
					Double res = this.calculNum(valeur);
					this.changeThis(new Arbre(res.toString()));
				}
			}
		}
	}
