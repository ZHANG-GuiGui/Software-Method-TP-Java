/* La classe ListeChaineeOrdonnee représente une liste d'entiers triee par ordre croissant.
 * La classe utilisera necessairement une liste chainee dont les elements sont des 
 * objets de la classe Chainon
...
public class ListeChaineeOrdonnee {
	 ...
	 * si la liste n'a qu'un maillon, debut et fin sont égaux
	 */
	...
	public void ajouter(int n) {
		if (debut == null) {
			debut = fin = new Chainon(n, null);
		}else if(debut==fin){
			if (n>debut.getDonnee()){
				fin = new Chainon(n,null);
				debut = new Chainon(debut.getDonnee(),fin);
			}else{
				debut = new Chainon(n,fin);
			}
		} else{
			Chainon mtn = debut;
			Chainon avt = null;
			while (n>mtn.getDonnee()){
				if (mtn==fin){
					break;
				}
				avt = mtn;
			...
			if (avt==null){
				debut = new Chainon(n,mtn);
			}else if (mtn==fin) {
				Chainon end = new Chainon(n,null);
				mtn.setSuivant(end);
				fin = end;
			}else{
				Chainon add = new Chainon(n,mtn);
				avt.setSuivant(add);
			}
		}
		// else
	...
	public int retirerPlusPetit() throws ExceptionListeVide {
		if ((debut == null)||(fin==null)){
			...
			if (debut.getSuivant()==fin){
				debut = new Chainon(fin.getDonnee(),null);
			}else if (debut.getSuivant()==null){
				debut = null;
				fin = null;
			}else{
